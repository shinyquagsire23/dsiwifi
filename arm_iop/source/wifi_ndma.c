...
void wifi_ndma_read(void* dst, u32 len)
{
    ...
    REG_NDMA_WRITE_CNT(WIFI_NDMA_CHAN) = len / 4;
    
    REG_NDMA_CNT(WIFI_NDMA_CHAN) = NDMA_ENABLE | NDMA_IMMEDIATE_MODE | NDMA_BLOCK_SIZE(1) | NDMA_SRC_MODE(NDMA_MODE_FIXED) | NDMA_DST_MODE(NDMA_MODE_INC);
...
void wifi_ndma_write(void* src, u32 len)
{
    ...
    REG_NDMA_CNT(WIFI_NDMA_CHAN) = NDMA_ENABLE | NDMA_IMMEDIATE_MODE | NDMA_BLOCK_SIZE(1) | NDMA_SRC_MODE(NDMA_MODE_INC) | NDMA_DST_MODE(NDMA_MODE_FIXED);
